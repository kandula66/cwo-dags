...
DELETE FROM {{ gold_db }}.vehicle_health_enriched
WHERE business_date = '{{ business_date }}';

INSERT INTO {{ gold_db }}.vehicle_health_enriched (
    source_file,
    vin,
    event_timestamp,
    event_date,
...
DELETE FROM {{ gold_db }}.daily_vehicle_health_summary
WHERE business_date = '{{ business_date }}';

INSERT INTO {{ gold_db }}.daily_vehicle_health_summary (
    business_date,
    event_date,
    country,
    customer_region,
...
DELETE FROM {{ gold_db }}.service_kpi_summary
WHERE business_date = '{{ business_date }}';

INSERT INTO {{ gold_db }}.service_kpi_summary (
    business_date,
    dealer_id,
    dealer_name,
    country,
...
DELETE FROM {{ gold_db }}.data_quality_report
WHERE business_date = '{{ business_date }}';

INSERT INTO {{ gold_db }}.data_quality_report (
    dataset_name,
    metric_name,
    metric_value,
    status,
