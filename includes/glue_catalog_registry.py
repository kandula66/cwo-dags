...
def _column_lines(columns_ddl: str) -> List[str]:
    return [line.strip() for line in columns_ddl.strip().split(",") if line.strip()]
...
def _glue_columns(columns_ddl: str, partition_columns: Sequence[str]) -> List[Dict[str, str]]:
    body = _data_columns_ddl(columns_ddl, partition_columns)
    columns: List[Dict[str, str]] = []
    for line in _column_lines(body):
        name, type_token = line.split(None, 1)
        columns.append({"Name": name, "Type": _parse_hive_type(type_token)})
    return columns
...
def _build_aws_clients(region: str) -> Tuple[GlueRestClient, S3RestClient]:
...
def register_curated_tables_standalone(
    ...
    """Register external tables in AWS Glue (no Spark session or explicit keys required)."""
    glue_client, s3_client = _build_aws_clients(region)
