...
def resolve_default_aws_credentials(region: str) -> AwsCredentials:
    ...
    raise RuntimeError(
        "No AWS credentials found in the runtime environment. Attach an IAM role to the "
        "Airflow worker (IRSA / instance profile) or ensure the platform injects "
        "AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY. Do not put access keys in DAG code."
    )
